...
export const emailSchema = yup.string().required("Email is required").email("Invalid email format");
...
export const selectSchema = (label: string) => yup.string().required(`${label} is required`);

export const checkboxSchema = yup.boolean().oneOf([true], "You must check this box");

export const multiSelectSchema = (min: number) =>
  yup.array().of(yup.string().required()).min(min, `Please select at least ${min} item(s)`);
...
export const timeSchema = yup
  ...
  .matches(/^([01]\d|2[0-3]):([0-5]\d)$/, "Invalid time format (24-hour format required)");
...
export const slackIdSchema = yup.string().required("Slack ID is required");
